...
pub use types::{GpuAllocation, AllocationType, GpuVendor, DetectionResult};
    ...
    pub fn detect_all(&self, pid: u32) -> Result<Vec<DetectionResult>> {
        ...
        for detector in &self.detectors {
            match detector.detect_allocations(pid) {
                Ok(result) => {
                    debug!("Detector {:?} found {} allocations for PID {}", 
                           detector.get_vendor(), result.allocations.len(), pid);
                    results.push(result);
                }
                Err(e) => {
                    warn!("Detector {:?} failed for PID {}: {}", 
                          detector.get_vendor(), pid, e);
                }
            }
        }
