...
use gpu_checkpoint::{
    ...
    checkpoint::{CheckpointEngine, CheckpointConfig, CheckpointStrategy},
    utils,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
            ...
            match format.as_str() {
                ...
                "human" => {
                    for result in &results {
                        ...
                        println!("Total GPU Memory: {}", utils::format_memory(result.total_gpu_memory));
                        ...
                        if cli.verbose {
                            ...
                            for (i, alloc) in result.allocations.iter().enumerate() {
                                ...
                                println!("      Address: 0x{:016x} - 0x{:016x}", 
        ...
        Commands::Checkpoint { pid, storage, strategy, bandwidth } => {
